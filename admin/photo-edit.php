...
if (isset($_POST['form1'])) {
	...
	if ($valid == 1) {
		...
		if ($path == '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_photo SET caption=?,url=? WHERE id=?");
			$statement->execute(array($_POST['caption'], $_POST['c_url'], $_REQUEST['id']));
		} else {
			unlink('assets/images/gallery/' . $_POST['previous_photo']);

			$final_name = 'photo-' . $_REQUEST['id'] . '.' . $ext;
			move_uploaded_file($path_tmp, 'assets/images/gallery/' . $final_name);

			// updating into the database
			$statement = $pdo->prepare("UPDATE tbl_photo SET caption=?, photo=?,url=? WHERE id=?");
			$statement->execute(array($_POST['caption'], $final_name, $_POST['c_url'], $_REQUEST['id']));
		}
					...
					<div class="box-body">
						<div class="form-group">
							...
							<div class="col-sm-3">
								<select class="form-control" name="caption">
									<option value="image" <?php if ($caption == 'image') {
																echo 'selected';
															} ?>>Image</option>
									<option value="video" <?php if ($caption == 'video') {
																echo 'selected';
															} ?>>Video</option>
								</select>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-6" style="padding-top:6px;">
								<img src="assets/images/gallery/<?php echo $photo; ?>" class="existing-photo" style="width:300px;">
						...
						<div class="form-group">
							...
							<div class="col-sm-4" style="padding-top:6px;">
								<input type="text" name="c_url" value="<?php echo $url; ?>" required>
							</div>
						</div>
						<div class="form-group">
